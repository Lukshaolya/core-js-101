...
function getPolynom(...coeffs) {
  ...
  return (x) => {
    ...
    for (let i = 0; i < coeffs.length; i += 1) {
      sum += coeffs[i] * x ** (coeffs.length - 1 - i);
    }
    return sum;
  };
}
...
function memoize(func) {
  ...
  return () => {
    ...
    const result = func.apply(this, ...func);
    cache[key] = result;
    return result;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  const partialArgs = Array.prototype.slice.call(...args1, 1);
  return function like() {
    const args = partialArgs.concat(Array.prototype.slice.call(...args1));
